...
/**
 * Form Rule Engine for Plank per FORM_RULES.md, EXERCISE_SPEC.md #7, and METRICS_SPEC.md §6.
 *
 * Implements:
 * 1. postural_break: instantaneous wobble outside [165°, 195°] that recovers before hold_end (severity 0.6).
 * 2. hips_dropping: hip_line_angle < 165° sustained >= 3 consecutive frames (severity 0.7).
 * 3. hips_piking: hip_line_angle > 195° sustained >= 3 consecutive frames (severity 0.5).
 ...
 */
internal class PlankFormRuleEngine(
    ...
    fun processFrame(
        ...
        val hipLineAngle = PlankGeometry.computeHipLineAngle(poseResult.landmarks)
    ...
    fun evaluateFrame(
        ...
        // 1. hips_dropping: hip_line_angle < 165° (severity 0.7)
        if (hipLineAngle < 165.0f) {
            val count = (consecutiveFrames[PlankFormRules.HIPS_DROPPING.errorName] ?: 0) + 1
            consecutiveFrames[PlankFormRules.HIPS_DROPPING.errorName] = count
            val err = FormError(
                errorName = PlankFormRules.HIPS_DROPPING.errorName,
                confidence = confidence,
                repIndex = null,
                severity = PlankFormRules.HIPS_DROPPING.severity
            )
            activeErrorsThisFrame.add(err)
            _allSessionErrors.add(err)
        } else {
            consecutiveFrames[PlankFormRules.HIPS_DROPPING.errorName] = 0
        }

        // 2. hips_piking: hip_line_angle > 195° (severity 0.5)
        if (hipLineAngle > 195.0f) {
            val count = (consecutiveFrames[PlankFormRules.HIPS_PIKING.errorName] ?: 0) + 1
            consecutiveFrames[PlankFormRules.HIPS_PIKING.errorName] = count
            val err = FormError(
                errorName = PlankFormRules.HIPS_PIKING.errorName,
                confidence = confidence,
                repIndex = null,
                severity = PlankFormRules.HIPS_PIKING.severity
            )
            activeErrorsThisFrame.add(err)
            _allSessionErrors.add(err)
        } else {
            consecutiveFrames[PlankFormRules.HIPS_PIKING.errorName] = 0
        }

        // 3. postural_break: instantaneous wobble outside [165°, 195°] that recovered or occurs during hold
        val isDeviating = abs(180.0f - hipLineAngle) > 15.0f
        if (isWobbleRecovered || (isHoldInProgress && isDeviating)) {
            val count = (consecutiveFrames[PlankFormRules.POSTURAL_BREAK.errorName] ?: 0) + 1
            consecutiveFrames[PlankFormRules.POSTURAL_BREAK.errorName] = count
            val err = FormError(
                errorName = PlankFormRules.POSTURAL_BREAK.errorName,
                confidence = confidence,
                repIndex = null,
                severity = PlankFormRules.POSTURAL_BREAK.severity
            )
            activeErrorsThisFrame.add(err)
            _allSessionErrors.add(err)
        } else if (!isDeviating) {
            consecutiveFrames[PlankFormRules.POSTURAL_BREAK.errorName] = 0
        }
        ...
        if (confidence >= confidenceThreshold) {
            ...
            if ((consecutiveFrames[PlankFormRules.HIPS_DROPPING.errorName] ?: 0) >= persistenceFrames) {
                candidateRules.add(PlankFormRules.HIPS_DROPPING)
            }

            // Check persistence rule for hips_piking (severity 0.5)
            if ((consecutiveFrames[PlankFormRules.HIPS_PIKING.errorName] ?: 0) >= persistenceFrames) {
                candidateRules.add(PlankFormRules.HIPS_PIKING)
            }

            // Check postural_break feedback eligibility (fires on wobble recovery)
            if (isWobbleRecovered) {
                candidateRules.add(PlankFormRules.POSTURAL_BREAK)
            }
        }
