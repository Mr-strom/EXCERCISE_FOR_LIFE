...
/**
 * Form Rule Engine for Shoulder Press per FORM_RULES.md and EXERCISE_SPEC.md.
 *
 * Implements:
 * 1. excessive_momentum: severity 0.55, feedback: "Control the movement, avoid swinging."
 *    Triggers when shoulder stability angle changes beyond threshold during the press.
 * 2. back_arching: severity 0.65, feedback: "Keep your back straight."
 *    Triggers when hip-shoulder vertical line deviates beyond threshold from vertical during the press.
 * 3. incomplete_lockout: severity 0.40, feedback: "Fully extend at the top."
 *    Triggers upon rep completion when top extension angle < 145° (top tolerance threshold).
 * 4. asymmetric_movement: severity 0.50, feedback: "Keep both sides even."
 *    Triggers when left vs right arm angle difference exceeds threshold during pressing.
 *    View-aware: strictly skips check when only one arm is visible (side view).
 * 5. insufficient_depth: severity 0.60, feedback: "Full range of motion."
 ...
 */
internal class ShoulderPressFormRuleEngine(
    ...
    val topLockoutThresholdDeg: Float = 145.0f
) {
    ...
    fun evaluateFrame(
        ...
        val isSideView = isSideViewOverride ?: if (landmarks.isNotEmpty()) {
            ShoulderPressGeometry.isSideView(landmarks)
        } else {
            false
        }
        ...
        if (isSideView) {
            ...
            consecutiveFrames[ShoulderPressFormRules.ASYMMETRIC_MOVEMENT.errorName] = 0
        } else {
            lastSkipReason = null
            val angleDiff = abs(leftElbowAngle - rightElbowAngle)
            val isAsymmetric = angleDiff >= asymmetricAngleThreshold && isRepInProgress

            val count = (consecutiveFrames[ShoulderPressFormRules.ASYMMETRIC_MOVEMENT.errorName] ?: 0) + (if (isAsymmetric) 1 else 0)
            consecutiveFrames[ShoulderPressFormRules.ASYMMETRIC_MOVEMENT.errorName] = if (isAsymmetric) count else 0

            if (isAsymmetric && count >= minPersistenceFrames) {
                val error = FormError(
                    errorName = ShoulderPressFormRules.ASYMMETRIC_MOVEMENT.errorName,
                    confidence = confidence,
                    repIndex = currentRepIndex,
                    severity = ShoulderPressFormRules.ASYMMETRIC_MOVEMENT.severity
                )
        ...
        // 2. Check Excessive Momentum (Torso / Shoulder Displacement)
        if (!isRepInProgress) {
            baselineTorsoAngle = shoulderStabilityAngle
            consecutiveFrames[ShoulderPressFormRules.EXCESSIVE_MOMENTUM.errorName] = 0
        } else {
            ...
            val count = (consecutiveFrames[ShoulderPressFormRules.EXCESSIVE_MOMENTUM.errorName] ?: 0) + (if (isMomentum) 1 else 0)
            consecutiveFrames[ShoulderPressFormRules.EXCESSIVE_MOMENTUM.errorName] = if (isMomentum) count else 0
        ...
        val archCount = (consecutiveFrames[ShoulderPressFormRules.BACK_ARCHING.errorName] ?: 0) + (if (isArching) 1 else 0)
        consecutiveFrames[ShoulderPressFormRules.BACK_ARCHING.errorName] = if (isArching) archCount else 0

        if (isArching && archCount >= minPersistenceFrames) {
            val error = FormError(
                errorName = ShoulderPressFormRules.BACK_ARCHING.errorName,
                confidence = confidence,
                repIndex = currentRepIndex,
                severity = ShoulderPressFormRules.BACK_ARCHING.severity
            )
            activeErrorsThisFrame.add(error)
            _allSessionErrors.add(error)
        }

        // 4. Rep-completion checks: incomplete_lockout and insufficient_depth
        if (completedRepMetrics != null && completedRepMetrics.repIndex != lastEvaluatedRepIndex) {
            ...
            // incomplete_lockout: top elbow angle doesn't reach top tolerance (< 145°)
            val peakAngle = maxOf(leftElbowAngle, rightElbowAngle)
            val isLockoutIncomplete = peakAngle < topLockoutThresholdDeg || completedRepMetrics.minElbowAngle < topLockoutThresholdDeg
            if (isLockoutIncomplete) {
                val lockoutError = FormError(
                    errorName = ShoulderPressFormRules.INCOMPLETE_LOCKOUT.errorName,
                    confidence = completedRepMetrics.confidence,
                    repIndex = completedRepMetrics.repIndex,
                    severity = ShoulderPressFormRules.INCOMPLETE_LOCKOUT.severity
                )
                activeErrorsThisFrame.add(lockoutError)
                _allSessionErrors.add(lockoutError)
            }

            // insufficient_depth: romPercent < 60%
            if (completedRepMetrics.romPercent < 60.0f) {
                val depthError = FormError(
                    errorName = ShoulderPressFormRules.INSUFFICIENT_DEPTH.errorName,
                    confidence = completedRepMetrics.confidence,
                    repIndex = completedRepMetrics.repIndex,
                    severity = ShoulderPressFormRules.INSUFFICIENT_DEPTH.severity
                )
        ...
        if (confidence >= confidenceThreshold && activeErrorsThisFrame.isNotEmpty()) {
            ...
            if (highestSeverityError != null) {
                val ruleDef = ShoulderPressFormRules.ALL_SHOULDER_PRESS_RULES.find { it.errorName == highestSeverityError.errorName }
