...
/**
 * Output Gate for Squat exercise analysis.
 *
 * Enforces:
 * 1. R7 refusal rules: strips all metrics when visibility is insufficient.
 * 2. Mathematical consistency between avgRepDurationSec and tutFactor:
 *    both derive from the exact same completed rep boundary timestamps (tutFactor = avgRepDurationSec / tutBaseline).
 * 3. Correct Form Factor formula per METRICS_SPEC.md §6:
 *    deduplicates form errors by (errorName, repIndex) so an ongoing error condition across multiple frames
 *    counts ONCE per rep, preventing disproportionate over-punishment.
 */
internal class SquatOutputGate(
    val config: ExerciseConfig = ExerciseConfig.SQUAT
) {

    /**
     * Assembles the final [SessionResult] object.
     */
    fun assembleSessionResult(
        ...
        // Best rep ROM% per DECISIONS.md D4
